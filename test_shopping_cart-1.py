...
class TestShoppingCart(unittest.TestCase):
    ...
    def test_remove_nonexistent_product(self):
        self.cart.add_product(self.product1)
        self.cart.remove_product("Orange")  # Trying to remove a product not in cart
        self.assertEqual(len(self.cart.products), 1)  # Cart should remain unchanged
        print("test_remove_nonexistent_product: Passed")

    def test_update_nonexistent_product_quantity(self):
        self.cart.add_product(self.product1)
        self.cart.update_quantity("Orange", 10)  # Trying to update quantity of a nonexistent product
        self.assertEqual(self.cart.products[0].quantity, 5)  # Quantity should remain unchanged
    ...
    def test_calculate_subtotal_with_zero_quantity(self):
        ...
        self.assertEqual(self.cart.calculate_subtotal(), 0.0)  # Subtotal should not include this product
